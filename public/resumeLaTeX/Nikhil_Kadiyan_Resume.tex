...
\pt Architected a \textbf{distributed, multi-tenant provisioning engine} using \textbf{Python, FastAPI, SQLAlchemy, Alembic, and Terraform} on \textbf{AWS (EC2, S3)} to automate isolated environment deployments across \textbf{3 global regions} (USA, Europe, Oceania), reducing onboarding deployment timeline from weeks to \textbf{under 2 hours} for \textbf{3k+ enterprise users}.\\
\pt Created \textbf{SpotterCode}, a custom \textbf{MCP server} handling \textbf{200 concurrent requests} via an async, multi-worker and horizontally scalable architecture with caching across \textbf{3 dedicated agents} (embed, docs, API); enabling \textbf{5k+ enterprise users} to embed BI websites via natural language, increasing embed adoption by \textbf{20\%}.\\
...
\pt Prototyped \textbf{Spotbuild}, an AI-powered prompt-to-BI system that auto-generates Liveboard answers and worksheets, pioneering an approach later adopted by the product's native AI features.\\
\sectionsep
...
\pt \textbf{Core:} Java, Golang, TypeScript, React, SQL, GraphQL, Python, C++ \\
\pt \textbf{Frameworks \& Libraries:} FastAPI, SQLAlchemy, Alembic, Next.js, Node.js, Flask \\
